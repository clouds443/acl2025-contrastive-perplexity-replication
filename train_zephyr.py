...
def main(args):
    ...
    for i in range(args.per_device_train_batch_size):
        idx_vec.append(torch.cat((torch.ones(int(args.num_pos),1),torch.zeros(args.num_neg,1),2*torch.ones(int(args.num_pos),1),3*torch.ones(args.num_neg,1)),dim=0))
                ...
                for cls, nums, curr_dict in zip(["POS", "TRANS", "NEG"],[int(self.num_pos), int(self.num_pos),  self.num_hard_negs], [self.pos_dict, self.neg_dict, self.neg_dict]):
                    ...
                    for i in range(nums):
                        ...
                        if cls == "TRANS": # translate NEG to REF
                            prompt = f'<|user|>\nParaphrase the following sentence, without using toxic or racist vocabulary: {item} </s>\n<|assistant|>\n{pos_item}'
                            proxy_target = tokenize(prompt)
                            prompt = f'<|user|>\nParaphrase the following sentence in a toxic way: {pos_item} </s>\n<|assistant|>\n{item}'
                            proxy_target = tokenize(prompt)
    ...
    class CustomTrainer(transformers.Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            if True:
                ...
                NUM_POS = args.num_pos*2 # must be dividble by 2
                ...
                loss_seq2seq_neg = torch.mean(loss,dim=1)[neg_idx]
                loss_seq2seq = torch.mean(loss_seq2seq_neg) + torch.mean(loss_seq2seq_pos)
